...
ble = bluetooth.BLE()
p = ble_library.BLESimplePeripheral(ble, "BLE_LJU")
...
def on_rx(v): 
    ...
    if v == '1':
        ...
        try:
            ...
            # 웹 브라우저가 행(Hang)에 걸리지 않도록 오류 상태 송신
            p.send("temp : --\n")
            p.send("humi : --\n")
    ...
    # '9' 수신 시: OLED에 Pochacco PBM 단색 비트맵 이미지 드로잉
    if v == '9':
        with open('img/pochacca.pbm', 'rb') as f:
...
while True:
    ...
    if cds_value > 2000 and cds_flag == 1:
        piezo.duty_u16(1000)
    ...
    elif cds_value <= 2000 and cds_flag == 0:
        motor.move(90)
        cds_flag = 1 
